...
def run_bfs(
    ...
    steps.append(
        AlgorithmStep(
            step=step_num,
            description=f"Start BFS from node {start_node}",
            visited_nodes=[],
            visited_edges=[],
    ...
    while queue:
        ...
        steps.append(
            AlgorithmStep(
                step=step_num,
                description=f"Visit node {node} (dequeued)",
        ...
        for neighbor, edge_id in adj.get(node, []):
            if neighbor not in visited_nodes and neighbor not in predecessors:
                ...
                steps.append(
                    AlgorithmStep(
                        step=step_num,
                        description=f"Discover neighbor {neighbor} via edge to {node}, add to queue",
                        visited_nodes=list(visited_nodes),
                        visited_edges=list(visited_edges),
                        current_node=node,
    ...
    if end_node and end_node in predecessors:
        path = _reconstruct_path(predecessors, end_node)
        result["path"] = path
        result["found"] = True
